...
class Reorder {
	...
	public function print_styles() {
		...
		if ( in_array( $pagenow, $pages ) )
			wp_enqueue_style( 'reorderpages_style', get_stylesheet_directory_uri() . '/css/admin/reorder.css' );
	...
	public function print_scripts() {
		...
		if ( in_array( $pagenow, $pages ) ) {
			wp_register_script( 'reorder_nested', get_stylesheet_directory_uri() . '/js/admin/jquery.mjs.nestedSortable.js', array( 'jquery-ui-sortable' ), '1.3.5', true );
			wp_enqueue_script( 'reorder_posts', get_stylesheet_directory_uri() . '/js/admin/reorder.js', array( 'reorder_nested' ) );
	...
	*/
	private function output_row( $the_post ) {
		...
		setup_postdata( $post );
		?>
		<li id="list_<?php the_id(); ?>"><div><?php the_title(); ?></div><!--span class="success">success</span--></li>
		<?php
	} //end output_row
